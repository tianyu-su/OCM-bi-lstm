...
def _to_sequence_example(set_info, decoder, vocab):
    ...
    for image_info in set_info['items']:
        filename = os.path.join(FLAGS.image_dir, str(image_info['item_id']) + '.jpg')
        ...
        # caption_id = [random.randint(1, 2700) for word in caption.split()]
    ...
    feature["set_url"] = _bytes_feature("url/{}".format(set_id))
    # Likes and Views are not used in our model, but we put it into TFRecords.
    feature["likes"] = _int64_feature(1)
    feature["views"] = _int64_feature(1)
